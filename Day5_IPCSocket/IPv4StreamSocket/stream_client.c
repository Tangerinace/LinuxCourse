...
#define handle_error(msg) \
    do { perror(msg); exit(EXIT_FAILURE); } while (0) // này cho mấy dòng if
		
/* Chức năng chat */
void chat_func(int server_fd) 
{
    int numb_write, numb_read;
    char recvbuff[BUFF_SIZE];  // giới hạn size nhận
    char sendbuff[BUFF_SIZE];
    while (1) {
        memset(sendbuff, '0', BUFF_SIZE); // tạo vòng lặp để chat liên tục
	   	memset(recvbuff, '0', BUFF_SIZE);
        printf("Please enter the message : ");
        fgets(sendbuff, BUFF_SIZE, stdin); // đọc và lưu tin nhắn vào sendbuff để gửi sang client
...
int main(int argc, char *argv[])
{
    int portno;
    int server_fd;
    struct sockaddr_in serv_addr; // Khai báo một biến có kiểu struct sockaddr_in tên là serv_addr.
	memset(&serv_addr, '0',sizeof(serv_addr));  
	
    /* Đọc portnumber từ command line */
    if (argc < 3) { // client,ip server,port
        printf("command : ./client <server address> <port number>\n");
        exit(1);
    }
    portno = atoi(argv[2]); // đối số thứ 2 ở đây là chỉ port no 8080
